...
def import_db_to_file(request):
    ...
    with connection.cursor() as cursor:
        ...
        text_query = get_query_to_import_new() #get_query_to_import()
        cursor.execute(text_query)
        data = cursor.fetchall()
...
-- Ключевые показатели эффективности (KPI) (+)															
-- Принятое решение
    t_pos_decision.positive_decision as positive_decision, -- Вид положительного решения (из списка) - динамика
    -- strftime('%d.%m.%Y',t_kpi.positive_decision_date) as positive_decision_date, -- ДАТА положительного решения - динамика
    
    -- Принятое решение
    strftime('%d.%m.%Y',t_kpi_positive_decision_fields_9.value) as positive_decision_date,  -- ДАТА положительного решения
...
--    t_kpi.settled_debt_amount as settled_debt_amount,
    t_kpi_positive_decision_fields_11.value as settled_debt_amount,
...
--    '' as pole3,
 ...
 LEFT JOIN positive_decision_fields as t_positive_decision_fields_8
  ...
  LEFT JOIN positive_decision_fields as t_positive_decision_fields_9
  ON t_kpi.positive_decision_type_id = t_positive_decision_fields_9.positive_decision_id
  AND t_positive_decision_fields_9.origin = 'pos_decision_date'
  LEFT JOIN kpi_positive_decision_fields as t_kpi_positive_decision_fields_9
  ON t_kpi.id = t_kpi_positive_decision_fields_9.kpi_id
  AND t_kpi_positive_decision_fields_9.fields_of_pos_decision_id = t_positive_decision_fields_9.id
  
  LEFT JOIN positive_decision_fields as t_positive_decision_fields_10
  ON t_kpi.positive_decision_type_id = t_positive_decision_fields_10.positive_decision_id
  AND t_positive_decision_fields_10.origin = 'measure_provided'
  LEFT JOIN kpi_positive_decision_fields as t_kpi_positive_decision_fields_10
  ON t_kpi.id = t_kpi_positive_decision_fields_10.kpi_id
  AND t_kpi_positive_decision_fields_10.fields_of_pos_decision_id = t_positive_decision_fields_10.id
  
  LEFT JOIN positive_decision_fields as t_positive_decision_fields_11
  ON t_kpi.positive_decision_type_id = t_positive_decision_fields_11.positive_decision_id
  AND t_positive_decision_fields_11.origin = 'settled_debt_amount'
